  ...
  return (
    <section className="hero-bloom relative overflow-hidden px-5 pb-24 pt-32">
      ...
      <div className="relative mx-auto max-w-5xl text-center">
        <Reveal>
          <span className="relative inline-flex items-center gap-2 overflow-hidden rounded-full border border-brand bg-badge px-4 py-1.5 text-[13px] font-medium text-paper">
            ✨ AI-Powered Content Repurposing for Writers
            <span
              aria-hidden="true"
              className="animate-shimmer absolute inset-y-0 left-0 w-1/3 bg-gradient-to-r from-transparent via-paper/25 to-transparent"
            />
          </span>
        </Reveal>

        <Reveal delay={80}>
          <h1 className="mt-8 text-[48px] font-extrabold leading-[1.05] tracking-[-1.5px] text-paper md:text-[72px] md:leading-[80px] md:tracking-[-2px]">
            <span className="block">One Post.</span>
            <span className="text-gradient-brand block drop-shadow-[0_0_28px_color-mix(in_oklab,var(--color-brand)_45%,transparent)]">
              Every Platform.
            </span>
          </h1>
        </Reveal>
        ...
        <Reveal delay={140}>
          <p className="mx-auto mt-6 max-w-2xl text-[18px] leading-7 text-lavender md:text-[20px] md:leading-8">

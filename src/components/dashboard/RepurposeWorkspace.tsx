...
export function RepurposeWorkspace({
  ...
  const mutation = useMutation({
    mutationFn: async () => generate({ data: { text, contentType, outputs: selected } }),
    onSuccess: (data) => {
      ...
      toast.success("Content generated! ✨", "Your outputs are ready to copy");
    },
    onError: () => {
  ...
  async function handleCopy() {
    try {
      ...
      sonnerToast.error("⚠️ Error: couldn't copy to clipboard.");
    }
  }
  ...
  async function handleRegenerate() {
    ...
    try {
      ...
      sonnerToast.error(message.replace(/^Error:\s*/, ""));
    } finally {
          ...
          {!results && !loading ? (
            <div className="flex min-h-[500px] flex-col items-center justify-center gap-5 p-6 text-center">
              ...
              <p className="max-w-md text-[15px] text-gray-muted">
                Paste your post on the left and click Reprose It to generate platform-native content
                in your voice.
              </p>
